#ifndef RAYCASTLIB_GBA_H
#define RAYCASTLIB_GBA_H
...
#include <tonc.h>
                             ...
                             yet. */
    #ifndef RCL_UNITS_PER_SQUARE
        #define RCL_UNITS_PER_SQUARE 512 /**< Number of RCL_Units in a side of a
                                            spatial square, i.e. the fixed point
                                            scaling. */
    #endif /* RCL_UNITS_PER_SQUARE */
...
#define RCL_HORIZONTAL_FOV_HALF (RCL_HORIZONTAL_FOV >> 1)

#ifndef RCL_CAMERA_COLL_RADIUS
    #define RCL_CAMERA_COLL_RADIUS (RCL_UNITS_PER_SQUARE >> 2)
#endif
    ...
    #define RCL_CAMERA_COLL_STEP_HEIGHT (RCL_UNITS_PER_SQUARE  >> 1)
#endif
...
#ifndef RCL_FLOOR_HEIGHT_FN
    #define RCL_FLOOR_HEIGHT_FN _floorHeightNotZeroFunction
#endif
...
typedef struct
{
    ...
    /*  Normalized (0 to RCL_UNITS_PER_SQUARE - 1)
        texture coordinates. */
    RCL_Vector2D  texCoords; 
} 
RCL_PixelInfo;

IWRAM_CODE static inline void RCL_PIXEL_FUNCTION(RCL_PixelInfo *pixel);
IWRAM_CODE static inline void RCL_COLUMN_FUNCTION(
    RCL_Unit x, 
    RCL_Unit yStart, 
    RCL_Unit yEnd, 
    u8 isFloor
);
IWRAM_CODE static inline RCL_Unit RCL_RS_HEIGHT_FN(s16 x, s16 y);
...
/**
  ...
  very often.
*/ 
typedef RCL_Unit (*RCL_ArrayFunction)(s16 x, s16 y);
/*
...
typedef void (*RCL_ColumnFunction)(
    RCL_HitResult *hits, 
    uint16_t hitCount, 
    uint16_t x,
    RCL_Ray ray
...
RCL_fast_div(RCL_Unit numerator, RCL_Unit denominator);

static inline RCL_Unit 
RCL_fast_mod(RCL_Unit numerator, RCL_Unit modulus);

inline RCL_Unit 
RCL_wrap(RCL_Unit value, RCL_Unit mod);
inline RCL_Unit 
RCL_divRoundDown(RCL_Unit value, RCL_Unit divisor);
...
RCL_castRayMultiHit(
    ...
    RCL_ArrayFunction typeFunc, 
    RCL_HitResult *hitResults,
    uint16_t *hitResultsLen, 
    RCL_RayConstraints constraints
);

static inline RCL_Vector2D RCL_angleToDirection(RCL_Unit angle);
...
u32 RCL_getFrame(void);
u16 *RCL_getDrawBuffer(void);


#endif /* RAYCASTLIB_GBA_H */

#ifdef RAYCASTLIB_GBA_IMPLEMENTATION
//=============================================================================
// privates
...
static RCL_Camera         _RCL_camera;
static RCL_Unit           _RCL_horizontalDepthStep     = 0; 
static RCL_Unit           _RCL_startFloorHeight        = 0;
static RCL_Unit           _RCL_startCeil_Height        = 0;
static RCL_Unit           _RCL_camResYLimit            = 0;
static RCL_Unit           _RCL_middleRow               = 0;
static RCL_ArrayFunction  _RCL_floorFunction           = 0;
static RCL_ArrayFunction  _RCL_ceilFunction            = 0;
static RCL_Unit           _RCL_fHorizontalDepthStart   = 0;
static RCL_Unit           _RCL_cHorizontalDepthStart   = 0;
static int16_t            _RCL_cameraHeightScreen      = 0;
static RCL_ArrayFunction  _RCL_rollFunction            = 0; // says door rolling
static RCL_Unit          *_RCL_floorPixelDistances     = 0;
static RCL_Unit           _RCL_fovCorrectionFactors[2] = {0,0}; //correction for hor/vert fov
static RCL_Unit           _RCL_fovScale                = 0;
static u32                 frame                       = 0;
static u16                *drawBuf;
static u8                  page                        = 0;
...
static inline RCL_Unit _RCL_fovCorrectionFactor(RCL_Unit fov);
...
pixelFunc(RCL_PixelInfo *p)
{
    ...
    if (p->isWall) {
        ...
        color = PAL_WALL;//3 - RCL_min(3, (int)(p->depth / RCL_UNITS_PER_SQUARE));
#else
    ...
    u16 *addr = drawBuf + ((p->position.y * SCREEN_W + p->position.x * 2) >> 1);
...
RCL_fast_div(RCL_Unit numerator, RCL_Unit denominator)
{
    if ((uint32_t)denominator < DEPTH_RECIPROCAL_SIZE)
        return (RCL_Unit)(((int64_t)numerator * depth_reciprocal[denominator]) >> 10);
...
RCL_fast_mod(RCL_Unit numerator, RCL_Unit modulus)
{
    if ((uint32_t)modulus < DEPTH_RECIPROCAL_SIZE) {
        register s32 prod = RCL_fast_div(numerator, modulus);
...
static inline
RCL_Unit 
RCL_clamp(RCL_Unit value, RCL_Unit valueMin, RCL_Unit valueMax)
{
    if (value >= valueMin)
    {
        if (value <= valueMax)
...
RCL_wrap(RCL_Unit value, RCL_Unit mod)
{
    ...
    return cmp * mod + RCL_fast_mod(value, mod) - cmp;
}
...
RCL_divRoundDown(RCL_Unit value, RCL_Unit divisor)
{
    return RCL_fast_div(value, divisor) - ((value >= 0) ? 0 : 1);
}

// Bhaskara's cosine approximation formula
#define trigHelper(x) (((RCL_Unit) RCL_UNITS_PER_SQUARE) *\
    ((RCL_UNITS_PER_SQUARE >> 1) * (RCL_UNITS_PER_SQUARE >> 1) - 4 * (x) * (x)) /\
    ((RCL_UNITS_PER_SQUARE >> 1) * (RCL_UNITS_PER_SQUARE >> 1) + (x) * (x)))
...
#else
    if (input < RCL_UNITS_PER_SQUARE >> 2)
        return trigHelper(input);
    else if (input < RCL_UNITS_PER_SQUARE >> 1)
        return -1 * trigHelper((RCL_UNITS_PER_SQUARE >> 1) - input);
    else if (input < 3 * RCL_UNITS_PER_SQUARE >> 2)
        return -1 * trigHelper(input - (RCL_UNITS_PER_SQUARE >> 1));
    else
...
{
    return RCL_cos(input - (RCL_UNITS_PER_SQUARE >> 2));
}
...
{
    return RCL_fast_div((RCL_sin(input) * RCL_UNITS_PER_SQUARE), RCL_nonZero(RCL_cos(input)));
}
...
{
    return RCL_fast_div((RCL_cos(input) * RCL_UNITS_PER_SQUARE), RCL_sin(input));
}
...
static inline
RCL_Unit 
RCL_dist(RCL_Vector2D p1, RCL_Vector2D p2)
{
    ...
    result = a + RCL_fast_div((44 * b), 102);
...
inline 
int8_t 
RCL_pointIsLeftOfRay(RCL_Vector2D point, RCL_Ray ray)
{
...
)
{
    ...
    *hitResultsLen = 0;
    ...
    if (ray.direction.x < 0) {
        step.x = -1;
        nextSideDist.x = RCL_fast_div(
                (RCL_wrap(ray.start.x,RCL_UNITS_PER_SQUARE) * delta.x),
                RCL_UNITS_PER_SQUARE
            );
    }
    else {
        ...
        nextSideDist.x = RCL_fast_div(
                ( (
                    RCL_wrap(
                            RCL_UNITS_PER_SQUARE 
                            - ray.start.x,RCL_UNITS_PER_SQUARE
                        ) ) * delta.x
                    ),
                RCL_UNITS_PER_SQUARE
            );
    ...
    {
        step.y = -1;
        nextSideDist.y = RCL_fast_div(
                (RCL_wrap(ray.start.y,RCL_UNITS_PER_SQUARE) * delta.y),
                RCL_UNITS_PER_SQUARE
            );
    }
    else
    {
        ...
        nextSideDist.y = RCL_fast_div(
                ( (
                    RCL_wrap(
                            RCL_UNITS_PER_SQUARE - ray.start.y,
                            RCL_UNITS_PER_SQUARE
                        )) * delta.y
                    ),
                RCL_UNITS_PER_SQUARE
    ...
    RCL_Unit dx = RCL_nonZero(ray.direction.x);
    RCL_Unit dy = RCL_nonZero(ray.direction.y);
    RCL_Unit rayDirXRecip = depth_reciprocal_65536[RCL_abs(dx)]; if (dx < 0) rayDirXRecip = -rayDirXRecip;
    RCL_Unit rayDirYRecip = depth_reciprocal_65536[RCL_abs(dy)]; if (dy < 0) rayDirYRecip = -rayDirYRecip;
    // ^ we precompute reciprocals to avoid divisions in the loop

    for (uint16_t i = 0; i < constraints.maxSteps; ++i) {
        RCL_Unit currentType = RCL_RS_HEIGHT_FN(currentSquare.x,currentSquare.y);
...
#define CORRECT(dir1,dir2)\
    RCL_Unit tmp = diff >> 2;        /* 4 to prevent overflow */ \
    h.distance = ((tmp >> 3) != 0) /* prevent a bug with small dists */ \
        ? RCL_fast_div((tmp * RCL_UNITS_PER_SQUARE * rayDir ## dir1 ## Recip), (RECIP_SCALE >> 2))\
...
static inline
RCL_HitResult 
RCL_castRay(RCL_Ray ray, RCL_ArrayFunction arrayFunc)
{
...
)
{
    ...
    dir1.x = RCL_fast_div((dir1.x * RCL_UNITS_PER_SQUARE), cos);
    dir1.y = RCL_fast_div((dir1.y * RCL_UNITS_PER_SQUARE), cos);

    dir2.x = RCL_fast_div((dir2.x * RCL_UNITS_PER_SQUARE), cos);
    dir2.y = RCL_fast_div((dir2.y * RCL_UNITS_PER_SQUARE), cos);
    ...
    for (int16_t i = 0; i < cam.resolution.x; ++i) {
        ...
        r.direction.x = dir1.x + RCL_fast_div(currentDX, cam.resolution.x);
        r.direction.y = dir1.y + RCL_fast_div(currentDY, cam.resolution.x);
...
static inline
RCL_Unit 
_RCL_floorCeilFunction(int16_t x, int16_t y)
{
...
static inline
RCL_Unit 
_floorHeightNotZeroFunction(int16_t x, int16_t y)
{
    return (RCL_RS_HEIGHT_FN(x,y) == 0) 
        ? 0 
    ...
    #define loop(doDepth,doCoords)\
        {\
            ...
            if (doCoords) { /*constant condition - compiler should optimize it out*/\
                ...
                _RCL_camera.position.x +  RCL_fast_div((d * dx), d2);\
                pixelInfo->texCoords.y =\
                _RCL_camera.position.y +  RCL_fast_div((d * dy), d2);\
            }\
...
#if RCL_COMPUTE_WALL_TEXCOORDS == 1
        pixelInfo->texCoords.y = RCL_fast_div(
                textureCoordScaled,
...
static inline 
void 
_RCL_makeInfiniteHit(RCL_HitResult *hit, RCL_Ray *ray)
{
    ...
    hit->position.x = RCL_fast_div(
            (ray->direction.x * hit->distance), 
            RCL_UNITS_PER_SQUARE
        );
    hit->position.y = RCL_fast_div(
...
)
{
    ...
    if (hitCount > 0)
    {
        ...
        else
        {
            ...
            RCL_Unit texCoordMod = RCL_fast_mod(hit.textureCoord, RCL_UNITS_PER_SQUARE);
            ...
            if (hitCount > 1) /* should probably always be true (hit on square
                                exit) */
            {
                if (RCL_fast_mod(hit.direction, 2) != RCL_fast_mod(hits[1].direction, 2))
                {
        ...
        if (goOn) {
            ...
            wallStart = _RCL_middleRow - RCL_fast_div(
                    (
            ...
            int16_t wallEnd =  _RCL_middleRow - RCL_fast_div(
                    (
    ...
    // draw ceiling
#ifdef RCL_COLUMN_FUNCTION
    RCL_COLUMN_FUNCTION(x, 0, wallStart - 1, 0);
    y = wallStart;
#else
    ...
    // draw floor
#ifdef RCL_COLUMN_FUNCTION
    RCL_COLUMN_FUNCTION(x, y, _RCL_camResYLimit, 1);
#else
    p.isWall = 0;
    ...
    _RCL_drawHorizontalColumn(
            ...
            _RCL_camera.height,
            1,
            RCL_COMPUTE_FLOOR_DEPTH,RCL_COMPUTE_FLOOR_TEXCOORDS,
            -1,
            &ray,
            &p
        );
...
IWRAM_CODE 
static inline 
void _RCL_precomputeFloorDistances(
    RCL_Camera camera,
    RCL_Unit *dest, 
    uint16_t startIndex
)
{
    RCL_Unit camHeightScreenSize =  RCL_fast_div(
            (camera.height * camera.resolution.y), 
            RCL_UNITS_PER_SQUARE
        );

    for (uint16_t i = startIndex; i < camera.resolution.y; ++i)
        dest[i] = RCL_perspectiveScaleVerticalInverse(
                camHeightScreenSize,
...
)
{
    ...
    uint16_t halfResY = cam.resolution.y >> 1;
    ...
    _RCL_horizontalDepthStep = RCL_fast_div(RCL_HORIZON_DEPTH, cam.resolution.y); 
...
)
{
    ...
    _RCL_camera        = cam;
    _RCL_camResYLimit  = cam.resolution.y - 1;
    _RCL_middleRow     = cam.resolution.y >> 1;
    _RCL_rollFunction  = rollFunc;

    _RCL_cameraHeightScreen = RCL_fast_div(
            (
                    _RCL_camera.resolution.y 
                    * (_RCL_camera.height - RCL_UNITS_PER_SQUARE)
                ),
            RCL_UNITS_PER_SQUARE
        );
    ...
    constraints.maxHits = (_RCL_rollFunction == 0)
    ...
    _RCL_fovScale = RCL_fast_div(
        _RCL_fovCorrectionFactors[1],
        RCL_UNITS_PER_SQUARE
    );
...
RCL_normalize(RCL_Vector2D v)
{
    ...
    result.x = RCL_fast_div((v.x * RCL_UNITS_PER_SQUARE), l);
    result.y = RCL_fast_div((v.y * RCL_UNITS_PER_SQUARE), l);
...
inline
RCL_Unit 
RCL_vectorsAngleCos(RCL_Vector2D v1, RCL_Vector2D v2)
{
    v1 = RCL_normalize(v1);
    v2 = RCL_normalize(v2);

    return RCL_fast_div((v1.x * v2.x + v1.y * v2.y), RCL_UNITS_PER_SQUARE);
}
...
)
{
    ...
    RCL_Unit middleColumn = camera.resolution.x >> 1;
    ...
    toPoint.x = RCL_fast_div((toPoint.x * cos - toPoint.y * sin), RCL_UNITS_PER_SQUARE); 
    toPoint.y = RCL_fast_div((tmp * sin + toPoint.y * cos), RCL_UNITS_PER_SQUARE); 

    result.depth = toPoint.x;

    result.position.x = middleColumn - RCL_fast_div(
            (
                    RCL_perspectiveScaleHorizontal(toPoint.y,result.depth) 
                    * middleColumn
                ),
            RCL_UNITS_PER_SQUARE
        );

    result.position.y = RCL_fast_div(
            (
...
{
    return RCL_fast_div((degrees * RCL_UNITS_PER_SQUARE), 360);
}
...
_RCL_fovCorrectionFactor(RCL_Unit fov)
{
  ...
  fov = RCL_min((RCL_UNITS_PER_SQUARE >> 1) - 1,fov);

  uint8_t index = fov >> 6;
  uint32_t t = ((fov - index * 64) * RCL_UNITS_PER_SQUARE) >> 6; 
  uint32_t v1 = table[index];
  uint32_t v2 = table[index + 1];
 
  return v1 + RCL_fast_div(((v2 - v1) * t), RCL_UNITS_PER_SQUARE);
}

IWRAM_CODE 
RCL_Unit 
RCL_perspectiveScaleVertical(RCL_Unit originalSize, RCL_Unit distance)
{
  if (_RCL_fovCorrectionFactors[1] == 0)
    _RCL_fovCorrectionFactors[1] = _RCL_fovCorrectionFactor(RCL_VERTICAL_FOV);
  ...
  return distance != 0 
        ? RCL_fast_div(
                ...
                RCL_nonZero(RCL_fast_div((_RCL_fovCorrectionFactors[1] * distance), RCL_UNITS_PER_SQUARE))
            ) 
        : 0;
}
  ...
  RCL_Unit scaledSize)
{
  ...
  return (scaledSize != 0)
        ? RCL_fast_div(
                ...
                RCL_nonZero( 
                        RCL_fast_div(
                                (_RCL_fovCorrectionFactors[1] * scaledSize), 
                                RCL_UNITS_PER_SQUARE
                            )
                    )
            ) 
...
RCL_perspectiveScaleHorizontal(RCL_Unit originalSize, RCL_Unit distance)
{
    ...
    return (distance != 0) 
        ? RCL_fast_div(
                ...
                RCL_nonZero(
                        RCL_fast_div(
                                (_RCL_fovCorrectionFactors[0] * distance), 
                                RCL_UNITS_PER_SQUARE
                            )
                    )
            )
...
)
{
  ...
  return scaledSize != 0 
    ? RCL_fast_div(
            (originalSize * RCL_UNITS_PER_SQUARE + (RCL_UNITS_PER_SQUARE >> 1)),
            RCL_fast_div(
                    (RCL_HORIZONTAL_FOV_TAN * 2 * scaledSize), 
                    RCL_UNITS_PER_SQUARE
                )
        )       
    : RCL_INFINITY;
...
)
{
    ...
    { \
        ...
        if ((currentHeight comp h ? currentHeight : h) \
            comp (height1 +  RCL_fast_div((hits[i].distance * heightDiff), distance))) \
        { \
            res =  RCL_fast_div((hits[i].distance * RCL_UNITS_PER_SQUARE), distance); \
            break; \
        } \
        currentHeight = h; \
        } \
...
#define collCheckOrtho(dir,dir2,s1,s2,x) \
    ...
    if (!dir##Collides) { /* now also check for coll on the neighbouring square */ \
        int16_t dir2##Square2 = RCL_divRoundDown( \
        ...
        if (xCollides || yCollides) {
            if (movesInPlane) {
                ...
                cornerNew.dir = (dir##Square) * RCL_UNITS_PER_SQUARE \
                        + (RCL_UNITS_PER_SQUARE >> 1) + dir##Dir \
                        * (RCL_UNITS_PER_SQUARE >> 1) - dir##Dir \
                ...
                newPos.x = RCL_max(
                        squarePos.x + RCL_CAMERA_COLL_RADIUS + 1,
                        RCL_min(
                                squarePos.x + RCL_UNITS_PER_SQUARE 
                                        - RCL_CAMERA_COLL_RADIUS - 1,
                                camera->position.x
                            )
                    );

                newPos.y = RCL_max(
                        squarePos.y + RCL_CAMERA_COLL_RADIUS + 1,
                        RCL_min(
                                squarePos.y + RCL_UNITS_PER_SQUARE 
                                        - RCL_CAMERA_COLL_RADIUS - 1,
                                camera->position.y
                            )
                    );
    ...
    if (computeHeight && (movesInPlane || (heightOffset != 0) || force))
    {
        ...
        int16_t xSquare1 = RCL_divRoundDown(
                camera->position.x - RCL_CAMERA_COLL_RADIUS,
                RCL_UNITS_PER_SQUARE
            );

        int16_t xSquare2 = RCL_divRoundDown(
                camera->position.x + RCL_CAMERA_COLL_RADIUS,
                RCL_UNITS_PER_SQUARE
            );

        int16_t ySquare1 = RCL_divRoundDown(
                camera->position.y - RCL_CAMERA_COLL_RADIUS,
                RCL_UNITS_PER_SQUARE
            );

        int16_t ySquare2 = RCL_divRoundDown(
                camera->position.y + RCL_CAMERA_COLL_RADIUS,
                RCL_UNITS_PER_SQUARE
            );
...
void 
RCL_initCamera(RCL_Camera *camera)
{
    camera->position.x   = 0;
    camera->position.y   = 0;
    camera->direction    = 0;
...
void 
RCL_initRayConstraints(RCL_RayConstraints *constraints)
{
...
u16 *
RCL_getDrawBuffer(void)
{
    return drawBuf;
}


#endif /* RAYCASTLIB_GBA_IMPLEMENTATION */
